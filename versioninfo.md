...
### 2021.11.21 v0.4.1 修复实时显示曲线数据不符合真实解调数据问题
...
### 2021.12.3 v0.4.1 修改对不同采样率显示图像的适配。
* 主要是`LEN_PER = FRE * TIMEINTERVAL * 1.34; -- mainwidget.cpp`，原本每次显示刷新更新的数据量应该是FRE * TIMEINTERVAL，但是实际测试发现程序的定时器的设定时间间隔是实际的4/3倍，为了防止显示队列溢出，向上取值1.34，目前测试30kHz和10kHz，无问题。
* 后续改进： 将程序中debug模式的代码和运行模式的整合，尽可能的共用代码，避免出现debug相关的函数修改了，忘记修改run模式代码，减小调试难度。
