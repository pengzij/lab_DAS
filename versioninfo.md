...
### 2021.11.26 v0.4.1 修复关闭解调未响应的bug，新增存储原始数据关闭显示，显示串口速率
...
### 2021.12.9 v0.4.1 优化不同频率显示问题
* 目前测试10kHz和30kHz显示无问题，同时更新10kHz高通滤波器。
* 由于实际测试QT的定时时间与实际不符因此在mainwidget.cpp `44: LEN_PER = FRE * TIMEINTERVAL * 1.34;`这里的系数1.34是因为定时器设置的时间于实际定时时间的1.34倍（实际是4/3向上取值），因此补全。
